...
rule extract_paired_reads_from_bam
	input: 
		"data/input/multi_refs/{sample}.bam"
	output:
...
rule bwa_map_paired_reads:
	...
	shell:
        "(bwa mem -M -R <read_group> -t {threads} {input.ref} {input.reads} | "
...
rule merge_alignments:
    input:
        paired_files = lambda wildcards: ["data/output/mapped_reads/{0}_paired.bam".format(stem) for stem in SAMPLE_FILES[wildcards.run_id][wildcards.sample]],
        singleton_files = lambda wildcards: ["data/output/mapped_reads/{0}_singletons.bam".format(stem) for stem in SAMPLE_FILES[wildcards.run_id][wildcards.sample]]
    output:
